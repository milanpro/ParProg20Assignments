...
struct hotspot *parse_hotspot_list(const char *hotspots_filename)
{
    ...
    while (getline(&line, &len, fp) != -1)
    {
        ...
        {
            hotspot_list = (hotspot *) malloc(sizeof(struct hotspot));
            new_hotspot = hotspot_list;
        }
        else
        {
            new_hotspot = (hotspot *) malloc(sizeof(struct hotspot));
...
struct coord *read_coords(const char *coords_filename)
{
    ...
    while (getline(&line, &len, fp) != -1)
    {
        if (last_coord == NULL)
        {
            coord_list = (coord *) malloc(sizeof(struct coord));
            new_coord = coord_list;
        }
        else
        {
            new_coord = (coord *) malloc(sizeof(struct coord));
